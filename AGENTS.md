...
2. **Identity bullets**, in order, the fields the inventory table mirrors:
   ...
   - `- Hardware:` the specific machine model, linked; internal components as 2-space sub-bullets when relevant, ordered processor, then RAM, then storage, then networking:
     ...
     - Networking: one sub-bullet per interface or port group, wired before wireless, formatted `<interfaz> (<zonas>)`. Name the port count and speed (`4 puertos Ethernet 1G`) or the Wi-Fi 802.11 standards (`Wi-Fi 802.11ax/b/g/n`). List each internal zone it serves backticked, joined in Spanish (e.g. `` (`LAN`, `GUEST` y `MEDIA`) ``); write the WAN uplink as plain `Internet` (not a zone code, matching the README diagram) and an unused interface or spare port as plain `disponible` (e.g. `(Internet y disponible)`, `` (`LAN` y disponible) ``, `(disponible)`).
