...
2. **Identity bullets**, in order, the fields the inventory table mirrors:
   ...
   - `- Hardware:` the specific machine model, linked; internal components as 2-space sub-bullets when relevant, ordered processor, then RAM, then storage, then networking:
     ...
     - Networking: one sub-bullet per interface or port group, wired before wireless, formatted `<interfaz> (<zonas>)`. Name the speed, prefixed with the port count only when there is more than one (`Ethernet 1G`, `2 Ethernet 2.5G`, `4 Ethernet 1G`), or the Wi-Fi 802.11 standards (`Wi-Fi 802.11ax/b/g/n`). List each internal zone it serves backticked, joined with commas (e.g. `` (`LAN`, `GUEST`, `MEDIA`) ``); write the WAN uplink as plain `Internet` (not a zone code, matching the README diagram) and an unused interface or spare port as plain `disponible` (e.g. `(Internet, disponible)`, `` (`LAN`, disponible) ``, `(disponible)`).
